...
import React, { useState, useEffect } from 'react';
import { FaChartLine, FaUsers, FaBook, FaQuestionCircle, FaTrophy, FaClock, FaFileExcel, FaFilePdf } from 'react-icons/fa';
import toast from 'react-hot-toast';
import reportService from '../../services/reportService';
import examService from '../../services/examService';
import exportService from '../../services/exportService';
...
const Reports = () => {
  ...
  const [exams, setExams]                           = useState([]);
  const [selectedExam, setSelectedExam]             = useState('');
  const [pdfLoading, setPdfLoading]                 = useState(null); // resultId being downloaded
  ...
  // ── PDF handler ──
  const handleDownloadPDF = async (resultId, studentName) => {
    try {
      setPdfLoading(resultId);
      toast.loading('កំពុងបង្កើត PDF...', { id: 'pdf' });
      const data = await exportService.downloadExamResultPDF(resultId);
      exportService.downloadFile(data, `លទ្ធផល_${studentName}_${Date.now()}.pdf`);
      toast.success('ទាញយក PDF បានជោគជ័យ!', { id: 'pdf' });
    } catch {
      toast.error('មិនអាចទាញយក PDF បានទេ', { id: 'pdf' });
    } finally {
      setPdfLoading(null);
    }
  };
              ...
              <div className="table-responsive">
                <table className="table table-hover mb-0">
                  ...
                  <tbody>
                    {examAnalytics.map((e, i) => (
                      <tr key={i}>
                        <td>{e.title}</td><td>{e.totalAttempts || 0}</td>
                        <td className="text-success">{e.passed || 0}</td>
                        <td className="text-warning">{e.average || 0}</td>
                        <td className="text-danger">{e.failed || 0}</td>
                        <td>{toFixed(e.averageScore)}%</td>
          ...
          <div className="card-body p-0">
            <div className="table-responsive">
              <table className="table table-hover mb-0">
                <thead className="table-light">
                  <tr>
                    <th>សិស្ស</th><th>អ៊ីមែល</th><th>ថ្នាក់</th>
                    <th>ការប្រឡង</th><th>ពិន្ទុ</th><th>ភាគរយ</th>
                    <th>ថ្ងៃប្រឡង</th>
                    <th style={{ width: 80 }}>PDF</th> {/* ✅ ថ្មី */}
                  </tr>
                </thead>
                <tbody>
                  {examResults.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => handleDownloadPDF(r.id, r.studentName)}
                  ...
                  {examResults.length === 0 && (
                    <tr><td colSpan="8" className="text-center py-4 text-muted">មិនទាន់មានទិន្នន័យ</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className="table-responsive">
              <table className="table table-hover mb-0">
                <thead className="table-light">
                  <tr>
                    <th>សិស្ស</th><th>អ៊ីមែល</th><th>ថ្នាក់</th>
                    <th>ប្រឡងបាន</th><th>ពិន្ទុសរុប</th><th>ពិន្ទុអាចបាន</th><th>មធ្យម</th>
                  </tr>
                </thead>
                <tbody>
                  {studentPerformance.map((s, i) => (
                    <tr key={i}>
                      <td>{s.fullName}</td><td>{s.email}</td>
                      <td><span className="badge bg-info text-dark">{s.className || '—'}</span></td>
                      <td>{s.totalExamsTaken || 0}</td>
                      <td>{s.totalScore || 0}</td>
                      <td>{s.totalPossible || 0}</td>
          ...
          <div className="card-body p-0">
            <div className="table-responsive">
              <table className="table table-hover mb-0">
                <thead className="table-light">
                  <tr>
                    <th>#</th><th>សិស្ស</th><th>អ៊ីមែល</th><th>ថ្នាក់</th>
                    <th>ចំនួនប្រឡង</th><th>ពិន្ទុមធ្យម</th><th>ពិន្ទុខ្ពស់</th>
                  </tr>
                </thead>
                <tbody>
                  {topStudents.map((s, i) => (
