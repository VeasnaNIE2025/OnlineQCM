...
const SubjectManagement = () => {
  ...
  const handleCloseModal = () => {
    setShowModal(false);
    setEditingSubject(null);
    setFormData({ name: '', description: '', teacherId: '', isActive: true });
  };
  ...
  const getTeacherName = (teacherId) => {
    ...
    return teacher ? `${teacher.fullName} (${teacher.email})` : 'មិនមានគ្រូ';
  };
              ...
              <div className={`card shadow-sm h-100 ${!subject.isActive ? 'bg-light' : ''}`}>
                <div className="card-body">
                  ...
                  <div className="mt-2 small text-muted">
                    👨‍🏫 {subject.teacherId ? getTeacherName(subject.teacherId) : 'មិនមានគ្រូ'}
                  </div>
                  <div className="mt-2">
                    <span className={`badge ${subject.isActive ? 'bg-success' : 'bg-secondary'}`}>{subject.isActive ? 'សកម្ម' : 'បិទ'}</span>
                  </div>
            ...
            <div className="modal-content">
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body">
                  ...
                  <div className="mb-3">
                    <label className="form-label">គ្រូបង្រៀន</label>
                    <select className="form-select" name="teacherId" value={formData.teacherId} onChange={handleChange}>
                      <option value="">-- គ្មាន (ជ្រើសរើសគ្រូ) --</option>
