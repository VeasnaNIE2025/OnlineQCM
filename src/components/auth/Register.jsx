...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ auto-generate username ពី email (part មុន @)
      const username = rest.email.split('@')[0];

      await register({ ...rest, username, role: 'student' });
      toast.success('ចុះឈ្មោះបានជោគជ័យ!');
      navigate('/login');
    } catch (error) {
    ...
    >
      ...
      >
        ...
        <div className="text-center mb-4">
          <div
            className="mx-auto mb-3 d-flex align-items-center justify-content-center rounded-4 text-white"
            style={{ width: '70px', height: '70px', background: '#0a4a38', fontSize: '30px' }}
          >
            📋
          </div>
          <h2 className="fw-bold mb-2">បង្កើតគណនីថ្មី</h2>
          <p className="text-muted small mb-0">ចុះឈ្មោះជាសិស្សថ្មីនៃប្រព័ន្ធ</p>
        </div>
        ...
        <p className="text-center mt-4 mb-0 small">
          មានគណនីរួចហើយ?{' '}
          <Link to="/login" className="text-decoration-none fw-semibold" style={{ color: '#0a4a38' }}>
            ចូលប្រព័ន្ធ
          </Link>
        </p>
